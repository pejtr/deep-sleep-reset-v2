  ...
  const handleAccept = async () => {
    ...
    try {
      const res = await fetch("/api/orders/upsell", {
        ...
        body: JSON.stringify({ upsell: "oto2", price: 17 }),
      });
    ...
    <div className="min-h-screen stars-bg px-4 py-10">
      <div className="max-w-md mx-auto text-center">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-white mb-3 leading-tight">
          Usni rychleji s řízenou meditací
        </h1>
        <p className="text-[oklch(0.65_0.04_265)] mb-6 text-sm">
          4 audio session speciálně navržené pro tvůj chronotyp. Vědecky podložené techniky
          pro rychlé usnutí a hluboký spánek.
        </p>
        ...
        <div className="bg-[oklch(0.12_0.025_265)] border border-[oklch(0.65_0.22_280/0.5)] rounded-2xl p-6 mb-6 text-left">
          ...
          <div className="space-y-3 mb-5">
            {[
              { icon: "🌙", title: "Deep Sleep Induction (23 min)", desc: "Progresivní svalová relaxace + delta wave guidance" },
              { icon: "🧠", title: "Mind Quieting Session (18 min)", desc: "Pro Dolphin a Wolf typy — ztišení hyperaktivní mysli" },
              { icon: "🌅", title: "Morning Energy Activation (12 min)", desc: "Ranní aktivace pro Lion a Bear typy" },
              { icon: "😴", title: "Power Nap Protocol (20 min)", desc: "Ideální polední spánek pro maximální odpolední výkon" },
            ].map((item, i) => (
        ...
        >
          {loading ? "Zpracovávám..." : "Ano! Chci Audio Pack za $17 →"}
        </button>

        <p className="text-xs text-[oklch(0.4_0.03_265)] mb-4">
          🔒 Jeden klik · Okamžitý přístup ke stažení
        </p>
        ...
        >
          Ne, děkuji — nepotřebuji audio meditace
        </button>
      </div>
    </div>
  );
